...
std::string Managers::ShaderManager::ReadShader(const std::string& filename) {
	...
	if (!file.good()) {
		spdlog::get("console")->error("Cannot Read Shader File {0}", filename.c_str());
		std::terminate();
	}
...
GLuint Managers::ShaderManager::CreateShader(GLenum shader_type, 
	const std::string& source, const std::string& shader_name) {
	...
	if (compile_result == GL_FALSE) {
		...
		spdlog::get("console")->error("Error compiling shader {0}", shader_name.c_str());
		spdlog::get("console")->error(&shader_log[0]);
		return 0;
	}
	return shader;
}

void Managers::ShaderManager::CreateProgram(const std::string& shader_name, 
	...
	if (link_result == GL_FALSE) {
		...
		spdlog::get("console")->error("Shader Loader LINK ERROR");
		spdlog::get("console")->error(&program_log[0]);
	}
...
const GLuint Managers::ShaderManager::GetShader(const std::string& shader_name) {
	...
	if (programs.find(shader_name) != programs.end()) {
		...
		spdlog::get("console")->error("Shader with name {0} does not exist", shader_name.c_str());
		return 0;
	}
}